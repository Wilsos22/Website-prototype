// =====================================================================
// BIG DOG MATH - WARM-UP → PROFICIENCY SPINE BRIDGE
...
//   BDM_EVIDENCE_KEY  — same value as EVIDENCE_INGEST_KEY in Vercel (required)
//   BDM_EVIDENCE_URL  — optional, defaults to https://bigdogmath.com/api/evidence
//
// What gets posted per submission:
//   1) ONE aggregate event: score 0–5 + the day's domain + the primary
...
// --- Form metadata (CCSS + distractor→misconception maps), saved at build time ---
...
// --- Topic → i-Ready domain (keyword matching; topics are free-typed) ---
...
function topicToDomain_(topic) {
  ...
  // ratios, fractions, decimals, GCF/LCM, percents, rates → Number and Operations
...
function postWarmupEvidenceSafely_(data, form, response) {
  try {
    return postWarmupEvidence_(data, form, response);
  } catch (err) {
...
function postWarmupEvidence_(data, form, response) {
  ...
  // Aggregate event: the day's 0–5 score → the domain bar + grouping.
