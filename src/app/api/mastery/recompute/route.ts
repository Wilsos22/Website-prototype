// Teacher-only (middleware basic-auth): rebuild mastery for a period from the
// raw logs — responses (warm-ups/tools) + checkpoint_results (Tier-1/Tier-2) —
// initialized from the i-Ready Fall baseline when present.
//
// POST /api/mastery/recompute  body: { periodId }
// Full rebuild (idempotent): recomputes every student's domain bars + per-standard
// stages via the golden-tested engine, rewrites mastery + mastery_history.
import { getSupabaseAdmin } from "@/lib/supabaseServer";
import {
  DOMAINS, TOPIC_DOMAIN, DEFAULT_CONFIG, scaleToMastery, warmupScoreToPct,
  replayDomains, deriveStage, ewmaStep, alphaFor,
  type Domain, type MasteryEvent, type EvidenceSource,
} from "@/lib/mastery";
...
export async function POST(req: Request) {
  ...
  if (!ids.length) return Response.json({ error: "No students in period." }, { status: 404 });
  ...
  // i-Ready Fall baseline (server-only table; missing rows fall back to 50).
  ...
  // Build per-student event lists.
  const domainEvents = new Map<string, MasteryEvent[]>();
  const stdEvents = new Map<string, Map<string, MasteryEvent[]>>(); // student → standard → events
  ...
  const cpAgg = new Map<string, CpAgg>(); // key: sid|cpId|('D:'domain or 'S:'ccss)
  for (const r of cpr || []) {
  ...
  for (const sid of ids) {
    ...
    // Domain bars: standard-scoped checkpoint events are already covered by the
    // domain-scoped aggregate rows, so exclude standardId events from the bar replay.
    ...
    // History: replay again, appending a row per event (plus the init point).
    const running = { ...init };
    for (const d of DOMAINS) {
    ...
    // Per-standard stages + EWMA percent (standards start at 0 — earned, not assumed).
  ...
  // Rewrite (full rebuild = idempotent).
  const del1 = await db.from("mastery_history").delete().in("student_id", ids);
  if (del1.error) return Response.json({ error: del1.error.message }, { status: 500 });
  const del2 = await db.from("mastery").delete().in("student_id", ids);
  if (del2.error) return Response.json({ error: del2.error.message }, { status: 500 });
