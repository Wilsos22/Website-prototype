...
export default function LessonPage() {
  ...
  useEffect(() => {
    if (!WARMUP_IDENTITY || !supabase || !sess?.sessionId || identityConfirmed) return;
    let stopped = false;
    const confirm = async () => {
      const { data } = await supabase.auth.getSession();
      ...
      setFirstName(result.session.name.trim().split(/\s+/)[0]);
  ...
  }, [identityConfirmed, sess?.sessionId, supabase]);
