...
const REEL_COLORS = ["#4e6ef2", "#22c55e", "#f59e0b"];
...
export default function StudentSpinner({ onClose }: { onClose?: () => void }) {
  ...
  return (
    <div className="sp-root">
      <style>{`
        .sp-root { min-height:100vh; width:100%; background:#0b0d14; color:#fff; font-family:Inter,ui-sans-serif,system-ui,sans-serif; display:grid; grid-template-rows:auto 1fr auto; }
        .sp-top { display:flex; align-items:center; justify-content:space-between; gap:8px; padding:14px 24px; border-bottom:1px solid #1f2332; flex-wrap:wrap; }
        .sp-mark { font-size:0.76rem; font-weight:900; letter-spacing:0.14em; text-transform:uppercase; color:#8ba0f8; margin:0; }
        .sp-tbtns { display:flex; gap:8px; flex-wrap:wrap; align-items:center; }
        .sp-sel { background:#121520; border:1px solid #2a3045; color:#fff; border-radius:8px; padding:8px 12px; font-weight:800; font-size:0.9rem; }
        .sp-btn { font-size:0.78rem; font-weight:800; letter-spacing:0.04em; color:#8a93ad; background:transparent; border:1px solid #1f2332; border-radius:7px; padding:8px 12px; cursor:pointer; text-decoration:none; }
        .sp-btn:hover { border-color:#4e6ef2; color:#fff; }
        .sp-toggle { display:inline-flex; border:1px solid #2a3045; border-radius:8px; overflow:hidden; }
        .sp-toggle button { background:#121520; border:none; color:#8a93ad; font-weight:800; font-size:0.82rem; padding:8px 12px; cursor:pointer; }
        .sp-toggle button.on { background:#4e6ef2; color:#fff; }
        ...
        .sp-reel-label { text-align:center; font-size:0.8rem; font-weight:900; letter-spacing:0.08em; text-transform:uppercase; margin:0 0 8px; }
        .sp-window { height:clamp(120px,18vh,190px); border-radius:16px; border:2px solid #1f2332; background:#121520; display:grid; place-items:center; padding:10px; overflow:hidden; position:relative; }
        ...
        .sp-name { font-size:clamp(1.4rem,3.6vw,2.5rem); font-weight:900; text-align:center; line-height:1.05; color:#fff; }
        .sp-name.blur { filter:blur(1.5px); opacity:0.7; }

        .sp-spin-btn { font-size:1.4rem; font-weight:900; letter-spacing:0.04em; color:#fff; background:#4e6ef2; border:none; border-radius:14px; padding:18px 48px; cursor:pointer; box-shadow:0 10px 30px -10px #4e6ef2; transition:transform 120ms ease, filter 140ms; }
        .sp-spin-btn:hover { filter:brightness(1.08); transform:translateY(-1px); }
        .sp-spin-btn:disabled { opacity:0.5; cursor:not-allowed; transform:none; }

        .sp-foot { padding:12px 24px; border-top:1px solid #1f2332; display:flex; align-items:center; justify-content:space-between; gap:12px; flex-wrap:wrap; }
        .sp-fair { display:flex; align-items:center; gap:8px; color:#8a93ad; font-size:0.85rem; font-weight:700; }
        .sp-hint { color:#5a6280; font-size:0.82rem; font-weight:700; }

        .sp-editor { background:#0d1018; border-top:1px solid #1f2332; padding:18px 24px; display:grid; gap:14px; }
        .sp-editor h3 { margin:0; font-size:0.82rem; font-weight:900; letter-spacing:0.06em; text-transform:uppercase; color:#8a93ad; }
        .sp-cols { display:grid; grid-template-columns:1fr 1fr; gap:18px; }
        @media (max-width:720px){ .sp-cols{ grid-template-columns:1fr; } }
        .sp-ta { width:100%; min-height:200px; background:#0b0d14; border:1px solid #2a3045; border-radius:10px; color:#fff; padding:12px; font-size:0.95rem; font-weight:600; line-height:1.6; resize:vertical; }
        .sp-ipad-list { display:grid; gap:6px; max-height:230px; overflow:auto; align-content:start; }
        .sp-ipad-row { display:flex; align-items:center; gap:9px; background:#121520; border:1px solid #1f2332; border-radius:8px; padding:7px 11px; font-weight:700; font-size:0.9rem; cursor:pointer; }
        .sp-ipad-row.on { border-color:#f59e0b; color:#fbbf24; }
        .sp-empty { color:#3a4460; font-weight:700; }
      `}</style>
